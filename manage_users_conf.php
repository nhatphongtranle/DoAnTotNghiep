            ...
            if ($row['username'] == "Name") {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql = "UPDATE users SET username=?, serialnumber=?, gender=?, email=?, myclass=?, user_date=CURDATE(), time_in=? WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                            } else {
                                mysqli_stmt_bind_param($result, "sdssss", $Uname, $Number, $Gender, $MyClass, $Email, $Timein);
                                mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_execute($result);
                                $sql = "INSERT INTO users ( username, serialnumber, gender, email, myclass, fingerprint_id, fingerprint_select, user_date, time_in, del_fingerid , add_fingerid) VALUES (?, ?, ?, ?, ?, ?, 1, CURDATE(), ?, 0, 1)";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error";
                                    exit();
                                } else {
                                    mysqli_stmt_bind_param($result, "sdsssis", $Uname, $Number, $Gender, $Email, $MyClass, $fingerid, $Timein);
                                    mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql = "UPDATE users SET username=?, serialnumber=?, gender=?, email=?, myclass=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
                                } else {
                                    mysqli_stmt_bind_param($result, "sdsss", $Uname, $Number, $Gender, $MyClass, $Email);
                                    mysqli_stmt_execute($result);
